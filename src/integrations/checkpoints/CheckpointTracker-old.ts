...
import { Controller as ApexProvider } from "../../core/controller"
import { fileExistsAtPath } from "../../utils/fs"
import { globby } from "globby"

class CheckpointTracker {
	private providerRef: WeakRef<ApexProvider>
	...
	private constructor(provider: ApexProvider, taskId: string, cwd: string) {
		this.providerRef = new WeakRef(provider)
		this.taskId = taskId
		this.cwd = cwd
	}

	public static async create(taskId: string, provider?: ApexProvider): Promise<CheckpointTracker | undefined> {
		try {
			if (!provider) {
			...
			const enableCheckpoints = vscode.workspace.getConfiguration("apex").get<boolean>("enableCheckpoints") ?? true
	...
	private static async getWorkingDirectory(): Promise<string> {
		...
		if (!cwd) {
			throw new Error("No workspace detected. Please open Apex in a workspace to use checkpoints.")
		}
	...
	public static async doesShadowGitExist(taskId: string, provider?: ApexProvider): Promise<boolean> {
	...
	public async initShadowGit(): Promise<string> {
		...
		if (await fileExistsAtPath(gitPath)) {
			...
			await git.addConfig("user.name", "Apex Checkpoint")
