import { ApexIgnoreController } from "./ApexIgnoreController"
...
describe("ApexIgnoreController", () => {
	let tempDir: string
	let controller: ApexIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .apexignore file
		await fs.writeFile(
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .apexignore file", async () => {
			const result = controller.validateAccess(".apexignore")
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .apexignore gracefully", async () => {
			// Create a new controller in a directory without .apexignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new ApexIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .apexignore", async () => {
			await fs.writeFile(path.join(tempDir, ".apexignore"), "")
