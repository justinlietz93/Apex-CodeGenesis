...
export class ContextManager {
	getNewContextMessagesAndMetadata(
		...
		if (shouldTruncate) {
			...
			if (needsTruncationBasedOnTokens || currentHistoryLength > 50) {
				console.log(`[ContextManager] Truncation needed. Reason: ${needsTruncationBasedOnTokens ? 'Token Limit Approaching' : 'History Length Exceeded'}. History length: ${currentHistoryLength}`);
	...
	public getNextTruncationRange(
	...
	): [number, number] | undefined { // Return undefined if no truncation needed
		...
		const finalMessagesToRemove = messagesToRemoveCount % 2 === 0 ? messagesToRemoveCount : Math.max(0, messagesToRemoveCount - 1);
		...
		if (newRangeEndIndex + 1 < apiMessages.length && apiMessages[newRangeEndIndex + 1]?.role === 'user' && newRangeEndIndex >= availableRangeStart) {
