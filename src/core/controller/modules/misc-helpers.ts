...
export async function fetchOpenGraphData(controller: Controller, url: string): Promise<void> {
    ...
    try {
        ...
        await postMessageToWebviewUtil(controller.webviewProviderRef, { type: "openGraphData", url: url, error: `Failed to fetch Open Graph data: ${error instanceof Error ? error.message : 'Unknown error'}` });
    }
}
