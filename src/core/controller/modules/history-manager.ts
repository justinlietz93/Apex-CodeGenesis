...
export async function exportTaskWithId(controller: Controller, taskId: string): Promise<void> {
	...
	try {
		...
		const markdownContent = apexMessages
			.map((message: ApexMessage) => {
				// Add type annotation
		...
		vscode.window.showErrorMessage(
			`Failed to export task ${taskId}: ${error instanceof Error ? error.message : "Unknown error"}`,
		)
	}
}
...
export async function deleteAllTaskHistory(controller: Controller): Promise<void> {
	...
	try {
		...
		await fs
			.rm(dummyTaskPath, { recursive: true, force: true })
			.catch((e) => console.warn("Could not remove dummy task dir", e))
