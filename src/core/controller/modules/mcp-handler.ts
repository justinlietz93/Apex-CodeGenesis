...
export async function fetchMcpMarketplace(controller: Controller, forceRefresh: boolean = false): Promise<void> {
    ...
    if (!MCP_FEATURE_ENABLED) {
        ...
        await postMessageToWebviewUtil(controller.webviewProviderRef, { type: "mcpMarketplaceCatalog", error: "MCP Marketplace feature is currently disabled." }); // (Rule #19: IMPL-UI)
        return;
    }
    ...
    const cachedCatalog = (cachedCatalogRaw && typeof cachedCatalogRaw === 'object' && Array.isArray((cachedCatalogRaw as McpMarketplaceCatalog).items))
    ...
    if (!forceRefresh && cachedCatalog) {
        ...
        await postMessageToWebviewUtil(controller.webviewProviderRef, { type: "mcpMarketplaceCatalog", mcpMarketplaceCatalog: cachedCatalog });
        return;
    }
    ...
    try {
        ...
        await postMessageToWebviewUtil(controller.webviewProviderRef, { type: "mcpMarketplaceCatalog", mcpMarketplaceCatalog: catalog });
        ...
        vscode.window.showErrorMessage(`Failed to fetch MCP marketplace: ${error instanceof Error ? error.message : 'Unknown error'}`);
        // Optionally send cached version if fetch fails but cache exists
        if (cachedCatalog) {
            await postMessageToWebviewUtil(controller.webviewProviderRef, { type: "mcpMarketplaceCatalog", mcpMarketplaceCatalog: cachedCatalog });
        } else {
             await postMessageToWebviewUtil(controller.webviewProviderRef, { type: "mcpMarketplaceCatalog", error: `Failed to fetch: ${error instanceof Error ? error.message : 'Unknown error'}` });
        }
    }
}
...
export async function downloadMcp(controller: Controller, mcpId: string): Promise<void> {
    ...
    await postMessageToWebviewUtil(controller.webviewProviderRef, { type: "mcpDownloadDetails", mcpDownloadDetails: { mcpId } as McpDownloadResponse }); // Send minimal info
    ...
    try {
        ...
        const catalog = (catalogRaw && typeof catalogRaw === 'object' && Array.isArray((catalogRaw as McpMarketplaceCatalog).items))
        ...
        if (!mcpItem || !(mcpItem as any).downloadUrl) { // Cast to any to check for assumed downloadUrl
        ...
        const installResult = { success: false, message: "Download/Install functionality not implemented.", serverName: mcpItem.name };
        ...
        const response: Partial<McpDownloadResponse> = { // Use Partial as we only have some fields
             mcpId: mcpItem.mcpId,
        ...
        await postMessageToWebviewUtil(controller.webviewProviderRef, { type: "mcpDownloadDetails", mcpDownloadDetails: response as McpDownloadResponse }); // Cast back

        // Report based on the (currently simulated) installResult
        if (installResult.success) {
            vscode.window.showInformationMessage(`MCP Server "${installResult.serverName || mcpItem.name}" installed successfully.`);
