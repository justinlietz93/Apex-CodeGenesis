...
import {
	...
	createProfile, deleteProfile, setActiveProfileId, updateProfile,
...
import { getVsCodeLmModels, getOllamaModels, getLmStudioModels, refreshOpenRouterModels, getOpenAiModels, readOpenRouterModels } from './modules/api-helpers'; // Keep this import
import { handleSignOut, setUserInfo, validateAuthState, handleAuthCallback, handleOpenRouterCallback } from './modules/auth-handler'; // Keep this import
import { addSelectedCodeToChat, addSelectedTerminalOutputToChat, fixWithApex } from './modules/context-actions'; // Keep this import
import { getTaskWithId, showTaskWithId, exportTaskWithId, deleteTaskWithId, deleteAllTaskHistory, updateTaskHistory, refreshTotalTasksSize } from './modules/history-manager'; // Keep this import
        ...
        try {
            switch (message.type) {
                ...
                case "webviewDidLaunch":
                    ...
                    readOpenRouterModels(this).then(models => models && postMessage({ type: "openRouterModels", openRouterModels: models })); // Use imported api-helper
                    getGlobalState(this.context, "justinlietz93.apex.mcpMarketplaceCatalog").then(catalog => catalog && postMessage({ type: "mcpMarketplaceCatalog", mcpMarketplaceCatalog: catalog as any }));
                    silentlyRefreshMcpMarketplace(this); // Use imported mcp-handler
                    refreshOpenRouterModels(this); // Use imported api-helper
                    // Use imported function directly
                    getStateToPostToWebview(this).then(state => telemetryService.updateTelemetryState(state.telemetrySetting === "enabled")); // Use imported state-updater
                    break;
                case "getLatestState":
                    await postStateToWebview(this); // Use imported function
                    break;
                ...
                case "apiConfiguration": // This likely comes from the SettingsView saving changes for the *active* profile
                    if (message.apiConfiguration) {
                        ...
                        if (this.task) {
                            ...
                            if (updatedConfig) { // Check if config exists (it should if a profile is active)
                ...
                case "telemetrySetting": // This is a global setting, not profile-specific
                 ...
                 case "updateSettings": // This message type might need adjustment for profiles
                    ...
                    if (message.planActSeparateModelsSetting !== undefined) {
                        ...
                        if (this.task) {this.task.planActSeparateModelsSetting = message.planActSeparateModelsSetting;}
                    }
                    if (message.chatSettings) {
                        settingsToUpdate.chatSettings = message.chatSettings;
                        if (this.task) {this.task.chatSettings = message.chatSettings;}
                    }
                     if (message.autoApprovalSettings) {
                        settingsToUpdate.autoApprovalSettings = message.autoApprovalSettings;
                        if (this.task) {this.task.autoApprovalSettings = message.autoApprovalSettings;}
                    }
                     if (message.browserSettings) {
                        settingsToUpdate.browserSettings = message.browserSettings;
                        if (this.task?.browserSession) {this.task.browserSession.browserSettings = message.browserSettings;}
                    }
                ...
                case "openExtensionSettings":
                    ...
                    await vscode.commands.executeCommand("workbench.action.openSettings", `@ext:justinlietz93.apex-ide-codegenesis ${settingsFilter}`.trim());
                    break;
                ...
                case "createProfile":
                    if (message.text) { // Assuming profile name is sent in 'text'
                    ...
                    break;
                case "deleteProfile":
                    if (message.text) { // Assuming profile ID is sent in 'text'
                ...
                case "updateProfile": // For renaming, etc. (not settings within profile)
                    if (message.profile) { // Assuming the updated UserProfile object is sent
                ...
                // --- NEW Custom Instruction Library Handlers ---
                 case "createCustomInstruction":
                    if (message.name && message.text) { // Assuming name and content are sent
                ...
                case "updateCustomInstruction":
                     if (message.customInstruction) { // Assuming updated CustomInstructionItem is sent
                        ...
                        if (this.task && state.activeProfileId && state.userProfiles) {
                            const activeProfile = state.userProfiles.find((p: UserProfile) => p.profileId === state.activeProfileId); // Added UserProfile type
                 ...
                 case "deleteCustomInstruction":
                    if (message.text) { // Assuming instruction ID is sent in 'text'
                ...
                case "accountLoginClicked":
                    ...
                    const authUrl = vscode.Uri.parse(`https://app.apex.bot/auth?state=${encodeURIComponent(nonce)}&callback_url=${encodeURIComponent(`${uriScheme || "vscode"}://justinlietz93.apex-ide-codegenesis/auth`)}`);
                ...
                case "checkpointDiff":
                    if (message.number) {await this.task?.presentMultifileDiff(message.number, false);}
                    break;
                case "checkpointRestore":
                    await cancelTask(this); // Use imported cancelTask which handles re-init
                    if (message.number && this.task) { // Check task exists after cancel/re-init
                         await pWaitFor(() => this.task?.isInitialized === true, { timeout: 3_000 }).catch(() => console.error("Timeout waiting for task re-initialization after cancel"));
                         if(this.task) {await this.task.restoreCheckpoint(message.number, message.text! as ApexCheckpointRestore);}
                    }
                    break;
                case "taskCompletionViewChanges":
                    if (message.number) {await this.task?.presentMultifileDiff(message.number, true);}
                    break;
                ...
                case "addRemoteServer":
                    try {
                        ...
                        await postMessage({ type: "addRemoteServerResult", addRemoteServerResult: { success: true, serverName: message.serverName! } });
                    } catch (error) {
                        await postMessage({ type: "addRemoteServerResult", addRemoteServerResult: { success: false, serverName: message.serverName!, error: error.message } });
                    }
                    break;
                case "showMcpView":
                ...
                case "fetchMcpMarketplace":
                    ...
                    break;
                case "downloadMcp":
                    if (message.mcpId) {await downloadMcp(this, message.mcpId);} // Use imported mcp-handler
                    break;
                case "silentlyRefreshMcpMarketplace":
                    await silentlyRefreshMcpMarketplace(this); // Use imported mcp-handler
                    break;
                case "toggleMcpServer":
                    await this.mcpHub?.toggleServerDisabled(message.serverName!, message.disabled!).catch(e => console.error(`Failed toggle MCP server ${message.serverName}:`, e));
                    break;
                case "toggleToolAutoApprove":
                    await this.mcpHub?.toggleToolAutoApprove(message.serverName!, message.toolNames!, message.autoApprove!).catch(e => console.error(`Failed toggle tool auto-approve for ${message.serverName}:`, e));
                    break;
                case "restartMcpServer":
                    await this.mcpHub?.restartConnection(message.text!).catch(e => console.error(`Failed restart MCP server ${message.text}:`, e));
                    break;
                case "deleteMcpServer":
                    if (message.serverName) {this.mcpHub?.deleteServer(message.serverName);}
                    break;
                case "fetchLatestMcpServersFromHub":
                 ...
                 case "updateMcpTimeout":
                     if (message.serverName && message.timeout) {
                        await this.mcpHub?.updateServerTimeout(message.serverName, message.timeout).catch(e => console.error(`Failed update timeout for ${message.serverName}:`, e));
                     }
                    break;
                ...
                case "invoke": // Generic invoke for UI actions
                    if (message.text) {await postMessage({ type: "invoke", invoke: message.text as Invoke });}
                    break;
    ...
    async togglePlanActModeWithChatSettings(chatSettings: ChatSettings, chatContent?: ChatContent) {
		...
		if (shouldSwitchModel) {
			...
			if (apiConfiguration) {
				...
				await updateGlobalState(this.context, "justinlietz93.apex.previousModeThinkingBudgetTokens", apiConfiguration.thinkingBudgetTokens)
				// ... (rest of the saving logic based on apiProvider) ...
				switch (apiConfiguration.apiProvider) {
					case "anthropic": case "bedrock": case "vertex": case "gemini": case "asksage": case "openai-native": case "qwen": case "deepseek":
						await updateGlobalState(this.context, "justinlietz93.apex.previousModeModelId", apiConfiguration.apiModelId); break;
					case "openrouter": case "apex":
						await updateGlobalState(this.context, "justinlietz93.apex.previousModeModelId", apiConfiguration.openRouterModelId);
						await updateGlobalState(this.context, "justinlietz93.apex.previousModeModelInfo", apiConfiguration.openRouterModelInfo); break;
					case "vscode-lm":
						await updateGlobalState(this.context, "justinlietz93.apex.previousModeVsCodeLmModelSelector", apiConfiguration.vsCodeLmModelSelector); break;
					case "openai":
						await updateGlobalState(this.context, "justinlietz93.apex.previousModeModelId", apiConfiguration.openAiModelId);
						await updateGlobalState(this.context, "justinlietz93.apex.previousModeModelInfo", apiConfiguration.openAiModelInfo); break;
					case "ollama":
						await updateGlobalState(this.context, "justinlietz93.apex.previousModeModelId", apiConfiguration.ollamaModelId); break;
					case "lmstudio":
						await updateGlobalState(this.context, "justinlietz93.apex.previousModeModelId", apiConfiguration.lmStudioModelId); break;
					case "litellm":
						await updateGlobalState(this.context, "justinlietz93.apex.previousModeModelId", apiConfiguration.liteLlmModelId); break;
					case "requesty":
						await updateGlobalState(this.context, "justinlietz93.apex.previousModeModelId", apiConfiguration.requestyModelId); break;
				}
			}
			...
			if (newApiProvider || newModelId || newThinkingBudgetTokens !== undefined || newVsCodeLmModelSelector) {
				...
				// ... (rest of the restoring logic based on newApiProvider) ...
                 switch (newApiProvider) {
                    case "anthropic": case "bedrock": case "vertex": case "gemini": case "asksage": case "openai-native": case "qwen": case "deepseek":
                        await updateGlobalState(this.context, "justinlietz93.apex.apiModelId", newModelId); break;
                    case "openrouter": case "apex":
                        await updateGlobalState(this.context, "justinlietz93.apex.openRouterModelId", newModelId);
                        await updateGlobalState(this.context, "justinlietz93.apex.openRouterModelInfo", newModelInfo); break;
                    case "vscode-lm":
                        await updateGlobalState(this.context, "justinlietz93.apex.vsCodeLmModelSelector", newVsCodeLmModelSelector); break;
                    case "openai":
                        await updateGlobalState(this.context, "justinlietz93.apex.openAiModelId", newModelId);
                        await updateGlobalState(this.context, "justinlietz93.apex.openAiModelInfo", newModelInfo); break;
                    case "ollama":
                        await updateGlobalState(this.context, "justinlietz93.apex.ollamaModelId", newModelId); break;
                    case "lmstudio":
                        await updateGlobalState(this.context, "justinlietz93.apex.lmStudioModelId", newModelId); break;
                    case "litellm":
                        await updateGlobalState(this.context, "justinlietz93.apex.liteLlmModelId", newModelId); break;
                    case "requesty":
                        await updateGlobalState(this.context, "justinlietz93.apex.requestyModelId", newModelId); break;
                }
				...
				if (this.task) {
					...
					if (updatedApiConfiguration) {
						...
						console.error("[Controller] Failed to build API handler after mode switch: No active API configuration found.");
		...
		if (this.task) {
			...
			if (this.task.webviewCommunicator?.isAwaitingPlanResponse && didSwitchToActMode) {
                if (this.task.webviewCommunicator) { // Check again for type safety
				    this.task.webviewCommunicator.didRespondToPlanAskBySwitchingMode = true;
                }
				await postMessageToWebviewUtil(this.webviewProviderRef, { // Use imported utility
					type: "invoke",
					invoke: "sendMessage",
