    ...
    try {
        ...
        switch (action) {
            case "launch":
                ...
                break;
            case "click":
                if (!params.coordinate) {return formatResponse.toolError(formatResponse.missingToolParameterError("coordinate"));}
                result = await task.browserSession.click(params.coordinate);
                break;
            case "type":
                if (!params.text) {return formatResponse.toolError(formatResponse.missingToolParameterError("text"));}
