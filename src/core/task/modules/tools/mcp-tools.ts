...
export async function executeUseMcpTool(
    ...
    if (!params.server_name) {return formatResponse.toolError(formatResponse.missingToolParameterError("server_name"));}
    if (!params.tool_name) {return formatResponse.toolError(formatResponse.missingToolParameterError("tool_name"));}
    if (!params.arguments) {return formatResponse.toolError(formatResponse.missingToolParameterError("arguments"));}
    ...
    try {
        ...
        return formatResponse.toolError(`Error using MCP tool ${params.tool_name} on server ${params.server_name}: ${error.message}`);
    }
}
...
export async function executeAccessMcpResourceTool(
    ...
    if (!params.server_name) {return formatResponse.toolError(formatResponse.missingToolParameterError("server_name"));}
    if (!params.uri) {return formatResponse.toolError(formatResponse.missingToolParameterError("uri"));}
    ...
    try {
        ...
        return formatResponse.toolError(`Error accessing MCP resource ${params.uri} on server ${params.server_name}: ${error.message}`);
    }
}
