...
export class StateManager {
	...
	async updateApiReqStartTime(index: number, userContent: UserContent): Promise<void> {
		if (index >= 0 && index < this.apexMessages.length && this.apexMessages[index].say === "api_req_started") {
			this.apexMessages[index].text = JSON.stringify({
				...
				request: userContent
					.map((block: Anthropic.ContentBlockParam) => formatContentBlockToMarkdown(block))
					.join("\n\n"),
	...
	async updateApiReqCompletionTime(
		index: number,
		metrics: Partial<Omit<ApexApiReqInfo, "request" | "cancelReason">> & { cancelReason?: ApexApiReqCancelReason } & {
			inputTokens?: number
			outputTokens?: number
		...
		// Adjusted type to include ApexApiReqCancelReason correctly
	...
	public async saveApexMessagesAndUpdateHistory() {
		...
		try {
			...
			if (controllerForHistoryUpdate) {
				await updateTaskHistory(controllerForHistoryUpdate, {
