...
export async function handleLimitChecks(
	...
	if (task.autoApprovalSettings.enabled && consecutiveAutoApprovedRequestsCount >= task.autoApprovalSettings.maxRequests) {
		if (task.autoApprovalSettings.enableNotifications) {
			showSystemNotification({
				subtitle: "Max Requests Reached",
				message: `Apex has auto-approved ${task.autoApprovalSettings.maxRequests.toString()} API requests.`,
			})
		}
