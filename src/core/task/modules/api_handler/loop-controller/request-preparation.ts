...
import { SelectPersonaParams, GetPersonaContentByNameRequest } from "../../../../../shared/BackendProtocol" // Adjust path
...
export async function prepareApiRequest(
	...
	if (!isContinuationFromNativeTool && userContent) {
		...
		await task.webviewCommunicator.say(
			"api_req_started",
			JSON.stringify({ request: "[Continuing after tool execution]\n\nLoading..." }),
		)
	}

	// --- Dynamic Persona Switching Check (Threshold Mode) ---
	const dynamicPersonaModeForCheck =
		vscode.workspace.getConfiguration("apex.agent").get<string>("dynamicPersonaMode") || "initial"
	if (dynamicPersonaModeForCheck === "threshold" && !isFirstRequest) {
		...
		if (currentTurnNumber > 0 && currentTurnNumber % checkFrequency === 0) {
			...
			try {
				...
				const personaNameResult = (await task.backendCommunicator.selectPersona(personaParams)) as unknown as {
				...
				if (suggestedPersonaName && suggestedPersonaName !== task.stateManager.currentActivePersonaName) {
					console.log(
						`[RequestPrep] Persona switch triggered: ${task.stateManager.currentActivePersonaName || "None"} -> ${suggestedPersonaName}`,
					)
					...
					if (contentResult.content !== null) {
						...
						console.warn(
							`[RequestPrep] Failed to fetch content for suggested persona ${suggestedPersonaName}. Switch aborted.`,
						)
					}
				...
				task.webviewCommunicator.say(
					"error",
					`Failed during persona switch check: ${error instanceof Error ? error.message : "Unknown error"}`,
				)
			}
		}
	}
