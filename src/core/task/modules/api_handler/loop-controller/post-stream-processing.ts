...
export async function handlePostStreamProcessing(
    ...
    if (streamResult.accumulatedNativeCalls.length > 0 && usedNativeToolsThisTurn) {
        ...
        for (const call of streamResult.accumulatedNativeCalls) {
             try {
                 ...
                 nativeToolResultParts.push({ functionResponse: { name: call.name, response: { content: typeof result === 'string' ? result : JSON.stringify(result) } } });
                 executedTool = true;
             } catch (error: any) {
                 console.error(`[PostStream] Error executing native tool ${call.name}:`, error);
                 nativeToolResultParts.push({ functionResponse: { name: call.name, response: { content: `[Tool Error]: ${error.message}` } } });
        ...
        const toolResultBlocks: Anthropic.ToolResultBlockParam[] = nativeToolResultParts.map((part, index) => {
             ...
             return { type: 'tool_result', tool_use_id: toolUseId, content: [{ type: 'text', text: responseContent, citations: null }], is_error: isError };
        });
    ...
    // Path 2: Tool Use Blocks (Anthropic Native & XML)
        ...
        for (const block of finalAssistantBlocks) {
            if (block.type === 'tool_use') {
                ...
                if (!toolUseId) { console.error("Tool use block missing ID:", toolUse); toolBlockIndex++; continue; }
                ...
                if (approvalStatus?.approved) {
                    try {
                        ...
                        const resultBlock: Anthropic.ToolResultBlockParam = {
                            ...
                            content: typeof result === 'string' ? [{ type: 'text', text: result || "(No output)", citations: null }]
                                : result.map((resBlock): Anthropic.TextBlockParam | Anthropic.ImageBlockParam | null => {
                                    if (resBlock.type === 'text') {return { type: 'text', text: resBlock.text ?? "", citations: null };}
                                    if (resBlock.type === 'image' && resBlock.source) {
                                        const source: Anthropic.ImageBlockParam.Source = { type: resBlock.source.type, media_type: resBlock.source.media_type, data: resBlock.source.data };
                                        return { type: 'image', source: source };
                                    }
                                    return { type: 'text', text: `(Unsupported tool output block type)`, citations: null };
                        ...
                        const errorResultBlock: Anthropic.ToolResultBlockParam = { type: 'tool_result', tool_use_id: toolUseId, content: [{ type: 'text', text: `[Tool Error]: ${errorMessage}`, citations: null }], is_error: true };
                    ...
                    toolResultBlocksForHistory.push({ type: 'tool_result', tool_use_id: toolUseId, content: [{ type: 'text', text: `[Tool Skipped: ${toolUse.name} - ${approvalStatus?.feedback || 'Not approved'}]`, citations: null }] });
                }
                toolBlockIndex++;
            }
        }
    ...
    if (executedTool && toolResultsContent && !recoveryNeededError) {
        ...
        telemetryService.captureConversationTurnEvent(task.taskId, task.apiProvider, task.api.getModel().id, toolResultsContent.role);
