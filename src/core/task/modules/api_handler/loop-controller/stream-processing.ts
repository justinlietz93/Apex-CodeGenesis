...
export async function processApiStream(
	...
	const updateApiReqMsg = async (cancelReason?: ApexApiReqCancelReason, streamingFailedMessage?: string) => {
		await task.stateManager.updateApiReqCompletionTime(lastApiReqIndex, {
			...
			cost:
				totalCost ??
				calculateApiCostAnthropic(task.api.getModel().info, inputTokens, outputTokens, cacheWriteTokens, cacheReadTokens),
			cancelReason,
	...
	const abortStream = async (cancelReason: ApexApiReqCancelReason, streamingFailedMessage?: string) => {
		...
		const abortReasonText: Anthropic.TextBlock = {
			type: "text",
		...
		if (
			abortMessageContent.length > 0 &&
			abortMessageContent.some(
				(b) => (b.type === "text" && b.text.trim() !== abortReasonText.text.trim()) || b.type !== "text",
			)
		) {
	...
	try {
		...
		try {
			// Inner try for the stream consumption loop
			for await (const chunk of stream) {
				...
				if (chunk.type === "function_calls" && supportsNativeTools) {
					...
					const calls = Array.isArray(functionCallsChunk.calls)
						? (functionCallsChunk.calls as FunctionCall[])
						: undefined
					if (calls) {
				...
				if (task.streamProcessor.didRejectTool) {
			...
			const finalCancelReason = streamError
				? streamError.message === "User cancelled" || streamError.message === "User rejected tool"
					? "user_cancelled"
					: "streaming_failed"
				: task.streamProcessor.didRejectTool
					? "user_cancelled"
					: undefined
			...
			if (controllerForStateUpdate) {
				await postStateToWebview(controllerForStateUpdate)
			}
		}
		...
		if (streamError && streamError.message !== "User cancelled" && streamError.message !== "User rejected tool") {
			if (!task.abandoned) {
				...
				if (controllerForHistory) {
					...
					if (history) {
						await initApexWithHistoryItem(controllerForHistory, history)
					}
				}
			}
