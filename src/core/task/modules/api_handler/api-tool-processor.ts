...
export class ApiToolProcessor {
		...
		if (streamResult.accumulatedNativeCalls.length > 0 && usedNativeToolsThisTurn) {
			...
			for (const call of streamResult.accumulatedNativeCalls) {
				...
				if (!toolUseNames.includes(toolName)) {
					...
					nativeToolResultParts.push({
						functionResponse: {
							name: call.name,
							response: { content: `[Tool Error]: Invalid tool name '${call.name}'` },
						},
					})
					continue
				}
			...
			const toolResultBlocks: Anthropic.ToolResultBlockParam[] = nativeToolResultParts.map((part, index) => {
				...
				const responseContent =
					responseObj && typeof responseObj === "object" && "content" in responseObj ? responseObj.content : undefined
			...
			!usedNativeToolsThisTurn &&
			finalAssistantBlocksFromStreamProcessor.some((b: AssistantMessageContent) => b.type === "tool_use")
		) {
				...
				if (block.type === "tool_use") {
					...
					if (approvalStatus?.approved) {
						try {
							...
							const result: ToolResponse = await this.task.toolExecutor.executeToolByName(
								toolUse.name,
								toolUse.params,
							)
			...
			// Add explicit type for 'b'
			const assistantTextContent = finalAssistantBlocksFromStreamProcessor.filter(
				(b: AssistantMessageContent): b is TextContent => b.type === "text",
			)
	...
	private formatAssistantContentForHistory(
