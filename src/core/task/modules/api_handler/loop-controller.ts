...
import {
    AnalyzeAndRecoverParams, RecoveryAction,
...
export async function recursivelyMakeApexRequests(
...
): Promise<boolean> { // Returns true if the loop *explicitly* decided to end here
    ...
    if (maxTaskTokens !== null && task.stateManager.totalTaskTokens >= maxTaskTokens) {
        console.warn(`[LoopController] Task token limit (${maxTaskTokens}) reached. Current total: ${task.stateManager.totalTaskTokens}. Pausing.`);
    ...
    if (streamResult.streamError) {
         if (streamResult.streamError.message !== "User cancelled" && streamResult.streamError.message !== "User rejected tool") {
             ...
             if (autonomyMode === 'full') {
                 console.warn(`[LoopController] Stream error occurred in full autonomy mode: ${streamResult.streamError.message}. Attempting recovery.`);
    ...
    if (detectedTaskCompletion) {
    ...
    } else if (isTaskComplete) { // Check existing flag if completion wasn't just detected
        ...
        if (recoveryNeededError || !shouldContinueLoop) { // Trigger recovery if error occurred OR no tool ran
            ...
            if (currentFailures > 3) { // Recovery attempt limit
                console.error(`[LoopController] Maximum recovery attempts (${currentFailures -1}) reached. Aborting task.`);
                await task.webviewCommunicator.say("error", `Agent failed to recover after ${currentFailures - 1} attempts. Task aborted.`);
                task.abortTask();
                explicitlyEndLoop = true;
            } else {
                ...
                console.log(`[LoopController] Full Autonomy: Attempting recovery (Attempt ${currentFailures}). Error: ${errorDescription}`);
                ...
                try {
                    ...
                    if (recoveryResult.next_actions && recoveryResult.next_actions.length > 0) {
                        ...
                        if (nextAction.type === "clarification_request") {
                            ...
                            await task.webviewCommunicator.say("info", `Recovery Analysis: ${recoveryResult.analysis}\nSuggested Question: ${nextAction.details?.question || 'Please provide guidance.'}`);
    ...
    if (explicitlyEndLoop) {
        ...
        if (controllerForEndLoopState) {await postStateToWebview(controllerForEndLoopState);} // Use imported function
        return true; // Signal loop end
         ...
         const nextUserContent = userContent && userContent.some(b => b.type === 'text' && b.text.startsWith('[Autonomous Recovery]:'))
             ? userContent
             : null; // Pass null if continuing after normal tool use

         return await recursivelyMakeApexRequests(
             task,
             consecutiveMistakeCount, setConsecutiveMistakeCount,
             consecutiveAutoApprovedRequestsCount, setConsecutiveAutoApprovedRequestsCount,
             didAutomaticallyRetryFailedApiRequest, setDidAutomaticallyRetryFailedApiRequest,
             consecutiveRecoveryFailures, setConsecutiveRecoveryFailures, // Pass recovery state
             autonomyMode, maxAutonomousSteps,
             autonomousStepsRemaining, setAutonomousStepsRemaining, // Pass updated remaining steps
             isTaskComplete, setIsTaskComplete,
         ...
         if (controllerForUnexpectedEnd) {await postStateToWebview(controllerForUnexpectedEnd);} // Use imported function
         return true;
    }
