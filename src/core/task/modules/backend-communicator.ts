...
import {
	...
	GeneratePlanParams, GeneratePlanResult,
	RefineStepsParams, RefineStepsResult,
	SelectPersonaParams, SelectPersonaResult,
	AnalyzeAndRecoverParams, AnalyzeAndRecoverResult,
	GetPersonaContentByNameRequest, GetPersonaContentByNameResult,
	ReplanningRequest, ReplanningResult, // Added replanning types
	KnowledgeSearchParams, KnowledgeSearchResult,
	RpcError
        ...
        try {
            ...
            try {
                await vscode.workspace.fs.stat(vscode.Uri.file(backendScriptPath));
                 ...
                 throw new Error(`Python backend script not found at expected location: ${backendScriptPath}. Stat error: ${statError}`);
            }
            ...
            this.pythonProcess.on("close", (code) => {
                ...
                if (!this.task.abort) { // Access abort via task
                     // Use webviewCommunicator
