...
import { ApexIgnoreController } from "../../core/ignore/ApexIgnoreController"
...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = apexIgnoreController ? apexIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, apexIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (apexIgnoreController && !apexIgnoreController.validateAccess(filePath)) {
		return null
	}
