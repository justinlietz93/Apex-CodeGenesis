...
export function createApexAPI(outputChannel: vscode.OutputChannel, sidebarController: Controller): ApexAPI {
	const api: ApexAPI = {
		...
		getCustomInstructions: async () => {
			return (await getGlobalState(sidebarController.context, "justinlietz93.apex.customInstructions")) as
				| string
				| undefined
		},
		...
		startNewTask: async (task?: string, images?: string[]) => {
			...
			await postMessageToWebview(sidebarController.webviewProviderRef, {
