...
} from '../core/task/modules/tools/mcp-tools'; // Adjust path
...
suite('MCP Tools Test Suite', () => {
    ...
    setup(() => {
        ...
        sandbox.stub(formatResponse, 'formatMcpResourceResult').callsFake(res => `Formatted Resource Result: ${JSON.stringify(res)}`);
    });
    ...
    suite('executeAccessMcpResourceTool', () => {
        ...
        test('Should return error if McpHub.readResource fails', async () => {
            ...
            const result = await executeAccessMcpResourceTool(task as any, { server_name: serverName, uri: uri });
            assert.strictEqual(result, `TOOL_ERROR: Error accessing MCP resource ${uri} on server ${serverName}: ${error.message}`);
        });
    });
});
