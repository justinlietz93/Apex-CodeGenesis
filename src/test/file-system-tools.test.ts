...
suite('File System Tools Test Suite', () => {
    ...
    suite('executeWriteToFileTool', () => {
        ...
        test('Should call DiffViewProvider correctly for new file', async () => {
            ...
            task.diffViewProvider.saveChanges.resolves({ finalContent: writeContent, userEdits: false, autoFormattingEdits: false, newProblemsMessage: null });
        ...
        test('Should return formatted response without user edits', async () => {
             ...
             task.diffViewProvider.saveChanges.resolves({ finalContent: writeContent, userEdits: undefined, autoFormattingEdits: mockAutoFormatDiff, newProblemsMessage: 'Formatted.' });
             // Pass string | undefined instead of boolean
             const expectedResponse = formatResponse.fileEditWithoutUserChanges(testFilePath, mockAutoFormatDiff, writeContent, 'Formatted.');
         ...
         test('Should return formatted response with user edits', async () => {
             ...
             task.diffViewProvider.saveChanges.resolves({ finalContent: 'User edited content', userEdits: mockUserDiff, autoFormattingEdits: undefined, newProblemsMessage: undefined });
             // Pass string | undefined instead of boolean
             const expectedResponse = formatResponse.fileEditWithUserChanges(testFilePath, mockUserDiff, undefined, 'User edited content', undefined);
    ...
    suite('executeReplaceInFileTool', () => {
        ...
        test('Should call dependencies and DiffViewProvider correctly', async () => {
            ...
            (diffUtils.constructNewFileContent as sinon.SinonStub).withArgs(diffContent, originalContent, true).resolves(newContent);
            task.diffViewProvider.saveChanges.resolves({ finalContent: newContent, userEdits: false, autoFormattingEdits: false, newProblemsMessage: null });
         ...
         test('Should return formatted response without user edits', async () => {
             ...
             // Mock with string | undefined for autoFormattingEdits
             const mockAutoFormatDiff = 'diff --git a/file b/file\n--- a/file\n+++ b/file\n@@ -1 +1 @@\n-old\n+new_formatted';
             task.diffViewProvider.saveChanges.resolves({ finalContent: newContent, userEdits: undefined, autoFormattingEdits: mockAutoFormatDiff, newProblemsMessage: 'Formatted.' });
             // Pass string | undefined instead of boolean
             const expectedResponse = formatResponse.fileEditWithoutUserChanges(testFilePath, mockAutoFormatDiff, newContent, 'Formatted.');
