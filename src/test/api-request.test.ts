...
class MockApiHandler implements ApiHandler {
    ...
    modelId = 'mock-model';
    config = {} as any;
    createMessage = sinon.stub().returns(async function* () { yield { type: 'text', text: 'Mock API response' }; }()); // Simple async generator
...
suite('attemptApiRequest Test Suite', () => {
    ...
    test('Should call selectPersona, generatePlan, and knowledgeSearch on first request', async () => {
        const generator = attemptApiRequest(task as any, -1, false, setDidAutomaticallyRetryFailedApiRequest, [{ type: 'text', text: 'Initial Task Goal' }]);
    ...
    test('Should handle API error and retry on user confirmation', async () => {
        ...
        task.api.createMessage.onFirstCall().returns(async function* () { throw apiError; }());
        // Simulate successful call on retry
        task.api.createMessage.onSecondCall().returns(async function* () { yield { type: 'text', text: 'Retry Success' }; }());
        ...
        assert.ok(task.webviewCommunicator.ask.calledOnceWith('api_req_failed', 'Simulated API Error'), 'Should ask user on API failure');
     ...
     test('Should handle API error and throw if user cancels retry', async () => {
        ...
        try {
            ...
            assert.ok(task.webviewCommunicator.ask.calledOnceWith('api_req_failed', 'Simulated API Error'), 'Should ask user on API failure');
            assert.ok(e.message.includes('API request failed and user did not retry'), 'Should throw specific error on user cancel');
        }
