import { useState, useRef, useCallback, useEffect, useLayoutEffect, useMemo } from 'react'; // Added useMemo
import { ContextMenuOptionType, getContextMenuOptions, insertMention, removeMention, shouldShowContextMenu, ContextMenuQueryItem } from '../../../utils/context-mentions'; // Added ContextMenuQueryItem
import { mentionRegex } from '../../../../../src/shared/context-mentions';
import { useEvent } from 'react-use';
import { vscode } from '../../../utils/vscode';
import { ExtensionMessage } from '../../../../../src/shared/ExtensionMessage';
import { GitCommit } from '../../../../../src/utils/git'; // Assuming this path is correct relative to src
...
export const useContextMentions = ({
  ...
  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
      if (showContextMenu) {
        ...
        if ((event.key === "Enter" || event.key === "Tab") && selectedMenuIndex !== -1) {
          ...
          if (selectedOption && selectedOption.type !== ContextMenuOptionType.URL && selectedOption.type !== ContextMenuOptionType.NoResults) {
    ...
    [
      showContextMenu, searchQuery, selectedMenuIndex, onMentionSelect, selectedType,
      inputValue, cursorPosition, setInputValue, justDeletedSpaceAfterMention, contextMenuOptions, textAreaRef // Use contextMenuOptions
    ]
  );
  ...
  const handlePaste = useCallback(async (e: React.ClipboardEvent) => {
    ...
    if (urlRegex.test(pastedText.trim())) {
      ...
      const currentPos = textAreaRef.current?.selectionStart ?? cursorPosition;
      const newValue = currentVal.slice(0, currentPos) + trimmedUrl + " " + currentVal.slice(currentPos);
      setInputValue(newValue);
