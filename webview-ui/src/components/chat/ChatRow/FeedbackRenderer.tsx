...
const FeedbackRenderer: React.FC<FeedbackRendererProps> = ({ message, isExpanded, onToggleExpand }) => {
	...
	if (message.say === "user_feedback_diff") {
		...
		try {
			...
			console.error("Failed to parse user feedback diff message:", message.text, e)
		...
		return (
			<div
				...
				<CodeAccordian diff={tool.diff} isFeedback={true} isExpanded={isExpanded} onToggleExpand={onToggleExpand} />
			</div>
		)
	}
