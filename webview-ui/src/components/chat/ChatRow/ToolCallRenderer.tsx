...
import { ApexMessage, ApexSayTool } from "../../../../../src/shared/ExtensionMessage"
...
const ToolCallRenderer: React.FC<ToolCallRendererProps> = ({ tool, message, isExpanded, onToggleExpand }) => {
	...
	switch (tool.tool) {
			...
			return (
				<>
					<div style={headerStyle}>
						{toolIcon("edit")}
						<span style={{ fontWeight: "bold" }}>Apex wants to edit this file:</span>
					</div>
					<CodeAccordian
		...
		case "newFileCreated":
			return (
				<>
					<div style={headerStyle}>
						{toolIcon("new-file")}
						<span style={{ fontWeight: "bold" }}>Apex wants to create a new file:</span>
					</div>
					<CodeAccordian
		...
		case "searchFiles":
			...
			const regexDisplay = tool.regex ? <code>{tool.regex}</code> : <span>an unspecified pattern</span>
			return (
				<>
					<div style={headerStyle}>
						{toolIcon("search")}
						<span style={{ fontWeight: "bold" }}>Apex wants to search this directory for {regexDisplay}:</span>
					</div>
					<CodeAccordian
						code={tool.content!}
		...
		default:
			...
			console.warn("Unknown tool type encountered in ToolCallRenderer:", tool.tool)
			return null
	}
}
