...
const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
  (
    ...
    const { filePaths, chatSettings, apiConfiguration, openRouterModels } =
      useExtensionState(); // Removed platform as it's used in ModeSwitch
    ...
    const updateHighlights = useCallback(() => {
      ...
      highlightLayerRef.current.scrollTop =
        internalTextAreaRef.current.scrollTop;
      highlightLayerRef.current.scrollLeft =
        internalTextAreaRef.current.scrollLeft;
    }, []);
    ...
    const handleContextButtonClick = useCallback(() => {
      ...
      const currentPos =
        internalTextAreaRef.current?.selectionStart ?? currentVal.length;
      ...
      if (!currentVal.trim() || currentVal.endsWith(' ')) {
        newValue =
          currentVal.slice(0, currentPos) + '@' + currentVal.slice(currentPos);
    ...
    const handleTextDrop = (text: string) => {
      ...
      const currentPos =
        internalTextAreaRef.current?.selectionStart ?? currentVal.length;
      const newValue =
        currentVal.slice(0, currentPos) + text + currentVal.slice(currentPos);
