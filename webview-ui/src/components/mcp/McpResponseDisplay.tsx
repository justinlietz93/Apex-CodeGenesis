  ...
  useEffect(() => {
    ...
    const processResponse = async () => {
      ...
      try {
        ...
        while (
          (urlMatch = urlRegex.exec(text)) !== null &&
          urlCount < MAX_URLS
        ) {
  ...
  const renderContent = () => {
    ...
    if (!isLoading) {
      ...
      if (urlMatches.length === 0) {
        segments.push(
          <UrlText key={`segment-${segmentIndex}`}>{responseText}</UrlText>
        );
      } else {
        for (let i = 0; i < urlMatches.length; i++) {
          ...
          segments.push(
            <UrlText key={`url-${segmentIndex++}`}>{fullMatch}</UrlText>
          );
...
const McpResponseDisplayWithErrorBoundary: React.FC<McpResponseDisplayProps> = (
  props
) => {
  return (
    <ChatErrorBoundary>
