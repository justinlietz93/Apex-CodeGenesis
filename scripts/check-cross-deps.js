...
function checkNpmConsistency(mainDeps, webviewDeps) {
	...
	if (inconsistencies.length > 0) {
		...
		for (const item of inconsistencies) {
			console.log(
				`  - ${colors.bright}${item.package}${colors.reset}: main(${colors.cyan}${item.mainVersion}${colors.reset}) vs webview(${colors.magenta}${item.webviewVersion}${colors.reset})`,
			)
		}
	} else {
...
function checkCrossLanguageDependencies(pythonDeps, mainDeps, webviewDeps) {
	...
	if (potentialIssues.length > 0) {
		...
		for (const issue of potentialIssues) {
			console.log(
				`  - Python: ${colors.cyan}${issue.pythonPackage}${colors.reset}@${issue.pythonVersion} / npm(${issue.environment}): ${colors.magenta}${issue.npmPackage}${colors.reset}@${issue.npmVersion}`,
			)
		}
  ...
  <div class="container">
    ...
    <section class="section">
      ...
      <table>
        ...
        <tbody>
          ${Object.entries(results.pythonDeps)
				.map(
					([pkg, version]) => `
            <tr>
      ...
      <table>
        ...
        <tbody>
          ${Object.entries(results.mainDeps)
				.map(
					([pkg, version]) => `
            <tr>
              <td><strong>${pkg}</strong></td>
      ...
      <table>
        ...
        <tbody>
          ${Object.entries(results.webviewDeps)
				.map(
					([pkg, version]) => `
            <tr>
              <td><strong>${pkg}</strong></td>
...
function main() {
	...
	console.log(
		`${colors.dim}Running on Node.js ${process.version} | ${process.platform}-${process.arch} | ${new Date().toISOString()}${colors.reset}\n`,
	)
