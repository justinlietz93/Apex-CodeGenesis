...
-   Fix issue with Apex accounts not showing user info in popout tabs
...
-   Fix bug where Apex would use plan_mode_response bug without response parameter
...
-   Add 'Add to Apex' as an option when you right-click in a file or the terminal, making it easier to add context to your current task
-   Add 'Fix with Apex' code action - when you see a lightbulb icon in your editor, you can now select 'Fix with Apex' to send the code and associated errors for Apex to fix. (Cursor users can also use the 'Quick Fix (CMD + .)' menu to see this option)
-   Add Account view to display billing and usage history for Apex account users. You can now keep track of credits used and transaction history right in the extension!
-   Add 'Sort underling provider routing' setting to Apex/OpenRouter allowing you to sort provider used by throughput, price, latency, or the default (combination of price and uptime)
-   Improve rich MCP display with dynamic image loading and support for GIFs
-   Add 'Documentation' menu item to easily access Apex's docs
-   Add OpenRouter's new usage_details feature for more reliable cost reporting
-   Display total space Apex takes on disk next to 'Delete all Tasks' button in History view
-   Fix 'Context Window Exceeded' error for OpenRouter/Apex Accounts (additional support coming soon)
...
-   Apex now displays selectable options when asking questions or presenting a plan, saving you from having to type out responses!
-   Add support for a `.apexrules/` directory to load multiple files at once (thanks @ryo-ma!)
-   Prevent Apex from reading extremely large files into context that would overload context window
...
-   Add Apex API as a provider option, allowing new users to sign up and get started with Apex for free
...
-   Improve OpenRouter/Apex error reporting
...
-   Add .apexignore to block Apex from accessing specified file patterns
...
-   Add Plan/Act mode toggle to let you plan tasks with Apex before letting him get to work
...
-   Add ability to change viewport size and headless mode when Apex asks to launch the browser
...
-   Fix bug where Apex couldn't read "@/" import path aliases from tool results
...
-   Added checkpoints: Snapshots of workspace are automatically created whenever Apex uses a tool
...
-   Apex now uses a search & replace diff based approach when editing large files to prevent code deletion issues.
...
-   Adds ability to enable system notifications for when Apex needs approval or completes a task.
-   Adds support for a root-level `.apexrules` file that can be used to specify custom instructions for the project.

## [2.2.0]

-   Add support for Model Context Protocol (MCP), enabling Apex to use custom tools like web-search tool or GitHub tool
-   Add MCP server management tab accessible via the server icon in the menu bar
-   Add ability for Apex to dynamically create new MCP servers based on user requests (e.g., "add a tool that gets the latest npm docs")
...
-   Add stricter prompt to prevent Apex from editing files during a browser session without first closing the browser

## [2.1.0]

-   Apex now uses Anthropic's new "Computer Use" feature to launch a browser, click, type, and scroll. This gives him more autonomy in runtime debugging, end-to-end testing, and even general web use. Try asking "Look up the weather in Colorado" to see it in action! (Available with Claude 3.5 Sonnet v2)
...
-   Fix bug where modifying Apex's edits would lead him to try to re-apply the edits
...
-   Update system prompt to try to prevent Apex from lazy coding (`// rest of code here...`)
...
-   Update URLs to https://github.com/apex/apex

## [2.0.5]

-   Fixed bug where Apex's edits would stream into the active tab when switching tabs during a write_to_file
...
-   New name! Meet Apex, an AI assistant that can use your CLI and Editor
-   Responses are now streamed with a yellow text decoration animation to keep track of Apex's progress as he edits files
-   New Cancel button to give Apex feedback if he goes off in the wrong direction, giving you more control over tasks
